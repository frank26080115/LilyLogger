...
void F_Fat::end(bool is_fs)
{
    if(_wl_handle != WL_INVALID_HANDLE){
        esp_err_t err;
        if (is_fs) {
            err = esp_vfs_fat_spiflash_unmount(_impl->mountpoint(), _wl_handle);
            if(err){
            ...
            err = wl_unmount(_wl_handle);
            if(err){
                log_e("Unmounting wear-leveling driver failed! Error: %d", err);
                return;
            }
        }
        _wl_handle = WL_INVALID_HANDLE;
